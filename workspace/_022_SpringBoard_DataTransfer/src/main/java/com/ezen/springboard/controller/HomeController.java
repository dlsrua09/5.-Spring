...
public class HomeController {
	...
	public String home(Locale locale, Model model) {
		...
		// viewResolver를 타고, 결국 /WEB-INF/views/home.jsp
		return "home";
	}
	
	@PostMapping("/")
	public String ezen(Model model) {
		// Model객체는 Request 영역에서 데이터를 저장할 수 있는 객체
	...
	// key값 "firstName"을 @RequestParam에서 꺼내서 firstName으로 받음
	// GetMapping("경로")
	@GetMapping("test.do")
	...
	// Command 객체는 메소드의 파라미터로 선언된 객체
	// public void test(UserVO user) => UserVO user = new userVO();
	// 파라미터로 넘어오는 데이터의 키값과 command 객체의 멤버변수의 이름이 같으면 
	// 그 멤머변수에 데이터의 값이 입력된다.
	...
	// 백단에서 화면단으로 데이터 전송
	// 1. Model 객체를 이용
	// model: 키는 String, 값은 Object로 되어있어서 어떠한 형태의 값이던지
	//        전송가능
	@GetMapping("/hello.do")
	public String hello(Model model, HttpServletRequest request, HttpSession session) {
//		model.addAttribute("hello", "12345"); 매개변수 (Model model)
...
//		
		...
		//    HttpSession은 session 끊킬 때까지 유지
	...
	public String getNameList(Model model) {
		...
		// List<Map<String, Object>>시, 
		// FIRST_NAME -> firstName 자동 변경됌. 
		// sql-map-config에 설정된 카멜케이스 떄문에
		List<NameVO> nameList = homeService.getNameList();
		
		// 가져온 값을 model 객체에 담아서 화면에 전달; key, value
	...
	public String deleteName(@RequestParam("nameNo") int nameNo, Model model) {
		...
		// model담을 때 key값=getNameList에 items="nameList" 여기에 담으면 된다.
...
//	}
	...
	public String updateName(@RequestParam Map<String, Object> paramMap) {
		...
		// redirect: 해당주소 => jsp를 찾아가지 않고, 해당주소를 다시 호출 @GetMapping("getNameList.do") 로 돌아감
		// redirect vs forward
		// redirect: WAS단에서 실행돼서 주소창의 url도 변경
		// forward: client(브라우저)단에서 실행돼서 주소창에 url이 변경X
